...
[VGG16 + Siamese](keras_face/library/siamese--SVM.py)
...
* VGG16 + Siamese: [keras_face/library/siamese.py](keras_face/library/siamese--SVM.py) is my own implementation of siamese network + VGG16 + 
...
def main():
    ...
    # verifies whether a particular camera image is a person in the image database 验证特定摄像机图像是否是图像数据库中的人
    ...
    # whether a particular camera image is which person in the image database (or not at all) 特定摄像机图像是图像数据库中的哪个人（或根本不是）
...
Below shows [sample codes](demo/siamese_demo_train.py) how to train the V166+Siamese network: 下面显示了[示例代码]（demo / siamese_demo_train.py）如何训练V166 + Siamese网络：
