...
def load_dataset(data_dir):
    ...
    classes = os.listdir(data_dir) # 通过数据集路径下文件夹的数量得到所有类别，返回指定的文件夹包含的文件或文件夹的名字的列表。这个列表以字母顺序
    category = 0 # 分类标签计数
...
# 建立facenet模型
facenet = load_model('./models/facenet_keras.h5') # bad marshal data (unknown type code)，用Python2实现的模型时会报这个错
#facenet.summary()
...
if __name__ == "__main__":
    images, labels = load_dataset('./data/dlib-align-images/')
    # 生成128维特征向量
