...
class SiameseFaceNet(object):
    ...
    def create_base_network(self, input_shape):
        ...
        x = Dropout(0.1)(x) #Tensor("dropout_1/cond/Merge:0", shape=(?, 128), dtype=float32)
        x = Dense(128, activation='relu')(x) #Tensor("dense_2/Relu:0", shape=(?, 128), dtype=float32)
        x = Dropout(0.1)(x) #Tensor("dropout_2/cond/Merge:0", shape=(?, 128), dtype=float32)
    ...
    def create_pairs(self, database, names):
        ...
        '''
        num_classes = len(database)#num_classes为参与训练的照片的张数
        pairs = []
        labels = []
    ...
    def verify(self, image_path, identity, database, threshold=None):
        ...
        identity -- string, name of the person you'd like to verify the identity. Has to be a resident of the Happy house.
                    字符串，您要验证身份的人的姓名。 必须是快乐屋的居民。
        database -- python dictionary mapping names of allowed people's names (strings) to their encodings (vectors).
                    python dictionary将允许的人的姓名（字符串）的名称映射到他们的编码（向量）。
        model -- your Inception model instance in Keras  Keras中的Inception模型实例

        Returns:
        dist -- distance between the image_path and the image of "identity" in the database.
                image_path与数据库中“identity”图像之间的距离。
        door_open -- True, if the door should open. False otherwise.  是的，如果门应该打开。 否则就错了。
        """
