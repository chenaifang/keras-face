...
class SiameseFaceNet(object):
    ...
    def __init__(self):
        self.model = None
        self.xception_include_top = True
        self.labels = None
        self.config = None
    ...
    def img_to_encoding(self, image_path):
        ...
        if self.xception_model is None:
            self.xception_model = self.create_xception_model()
        ...
        #print(type(image))
        img = cv2.resize(image, (299, 299), interpolation=cv2.INTER_AREA)#改变图片大小，修改为224*224
        ...
        return self.xception_model.predict(input)
    ...
    def load_model(self, model_dir_path):
        ...
        self.xception_include_top = self.config['xception_include_top']
    ...
    def fit(self, database, model_dir_path, epochs=None, batch_size=None, threshold=None, xception_include_top=None):
        '''
        ...
        if epochs is None:
            epochs = 60
        if xception_include_top is not None:
            self.vgg16_include_top = xception_include_top
        ...
        print("=======================")
        print(self.xception_include_top)
        self.xception_model = self.create_xception_model() #self.vgg16_model是在init函数中定义的
        ...
        self.config['xception_include_top'] = self.xception_include_top
